...
namespace Jellyfin.Plugin.SmartLists.Core.Orders
{
    ...
    {
        // Must stay in sync with NameSortHelper.EpisodeAutoSortPattern - both detect the same
        // Jellyfin-generated episode sort name, so \s+ (any whitespace run), not literal spaces.
        private static readonly Regex AutoGeneratedSortNamePattern =
            new(@"^\d{3,}\s+-\s+\d{4,}\s+-\s+", RegexOptions.Compiled);
