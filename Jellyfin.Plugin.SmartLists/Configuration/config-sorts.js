(function(SmartLists) {
    ...
    SmartLists.shouldShowSortOption = function(sortValue, selectedMediaTypes, hasSimilarToRule) {
        ...
        const isLiveTvOnly = hasLiveTvChannel && selectedMediaTypes.length === 1;

        // Live TV channel - hide sort options that don't apply
        if (isLiveTvOnly) {
            if (sortValue === 'ProductionYear' || sortValue === 'CommunityRating' ||
                sortValue === 'ReleaseDate' || sortValue === 'Runtime' || sortValue === 'Resolution') {
                return false;
            }
        }

        // ChannelResolution - only show when LiveTvChannel is selected
        if (sortValue === 'ChannelResolution') {
