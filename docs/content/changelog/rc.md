...
- A fully watched series is no longer reported as **In Progress**, which could place it on the wrong side of a **Playback Status** rule.
...
- Because Similar To no longer filters the whole list, lists combining a Similar To rule with *other* rule blocks may gain items: the other blocks' matches are no longer discarded for being insufficiently similar, and a Similar To-only block now contributes its own matches. Lists with several Similar To rules in *different* blocks now score each block against its own reference items instead of one blended pool. Lists whose Similar To rules all sit in a single block (the common case) are unchanged.
