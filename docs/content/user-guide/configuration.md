...
- **Automatic Image Generation**: Playlist covers are generated by SmartLists from the playlist's items (see details below)
...
SmartLists automatically generates cover images for playlists and collections when no custom images have been provided. For collections, auto-generation occurs only if:
...
- Playlist covers are generated by SmartLists itself (instead of Jellyfin's built-in playlist image generation), so they are consistent with collection covers and can carry the smart list badge
- A square 2x2 collage is created from the first four distinct item images in the playlist (album art is used for audio tracks, series posters for episodes)
- If only one distinct image is available, it is used as the cover: a square center-cropped badged copy when the smart list badge is enabled, or the untouched original image when the badge is disabled
- To use your own cover, upload a Primary image through SmartLists, or directly in Jellyfin (both are preserved — auto-generation only runs when neither exists)

##### Collection Primary Images (Vertical Posters)
...
##### Collection Thumb Images (Horizontal/Landscape)
...
- Playlist and collection images are automatically regenerated when the list is refreshed to reflect the current items

#### Smart List Badge

To make smart playlists and collections easy to spot when browsing, SmartLists stamps a small badge (the plugin's gear logo) in the top-left corner of cover images:

- Applied to auto-generated covers as well as Primary and Thumb images uploaded through SmartLists
- Uploaded originals are stored untouched — the badge is only stamped on the copy handed to Jellyfin, so toggling the badge off restores clean covers
- Can be turned off in the Settings tab of the admin page (**Show smart list badge on covers**); changes take effect on each list's next refresh
- Image formats that cannot be re-encoded (e.g. SVG) are used as-is without a badge

!!! note "Covers are cropped to Jellyfin's tile proportions while the badge is enabled"
    Jellyfin overviews crop covers to fixed tile shapes (square for playlists, 2:3 posters and 16:9 thumbs for collections), while detail pages show the image at its original proportions — and off-ratio covers also stretch their tile and make overview grids uneven. A badge stamped in the corner of an off-ratio image would be cut away by the overview crop; there is no position that works for every view. So while the badge is enabled, covers (including images uploaded through SmartLists) are center-cropped at native resolution to the matching tile shape before the badge is stamped: playlist Primary covers to a square, collection Primary covers to 2:3, and Thumb images to 16:9. Disable the badge to keep covers at their original proportions — uploads are then handed to Jellyfin untouched. Your uploaded originals are always stored uncropped, so toggling the badge off restores them on the next refresh.
