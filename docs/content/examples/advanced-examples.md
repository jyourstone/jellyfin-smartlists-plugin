...
### Interleaved Music Genre Journey
Create a music playlist that alternates between genres throughout, taking you on a sonic journey:

- **OR Block 1** (Energetic Rock):
  - **Genre** contains "Rock"
  - **Community Rating** >= 7
  - **Max Items for this OR block**: 15
- **OR Block 2** (Smooth Jazz):
  - **Genre** contains "Jazz"
  - **Community Rating** >= 7
  - **Max Items for this OR block**: 15
- **OR Block 3** (Classical Elegance):
  - **Genre** contains "Classical"
  - **Community Rating** >= 7
  - **Max Items for this OR block**: 15
- **Sort by**: 
  - **Primary**: Rule Block Order Interleaved (Ascending)
  - **Secondary**: Community Rating descending
- Result: A 45-track playlist that alternates: top rock → top jazz → top classical → 2nd best rock → 2nd best jazz → 2nd best classical... creating a constantly shifting sonic landscape with the highest-rated tracks from each genre

### Complex Runtime-Balanced Interleave
Create a perfectly balanced playlist where each genre contributes equally in both quantity AND runtime:

- **OR Block 1** (Short Action Clips):
  - **Genre** contains "Action"
  - **Runtime** <= 100 minutes
  - **Max Items for this OR block**: 20
- **OR Block 2** (Short Comedies):
  - **Genre** contains "Comedy"
  - **Runtime** <= 100 minutes
  - **Max Items for this OR block**: 20
- **OR Block 3** (Short Dramas):
  - **Genre** contains "Drama"
  - **Runtime** <= 100 minutes
  - **Max Items for this OR block**: 20
- **Sort by**: 
  - **Primary**: Rule Block Order Interleaved (Ascending)
  - **Secondary**: Runtime ascending
  - **Tertiary**: Community Rating descending
- **Global Max Items**: 30
- Result: A 30-item playlist alternating between genres, progressing from shortest to longest within each genre, with highest-rated content prioritized when runtimes match. Pattern: shortest action → shortest comedy → shortest drama → 2nd shortest action... ensuring variety in both genre and pacing
